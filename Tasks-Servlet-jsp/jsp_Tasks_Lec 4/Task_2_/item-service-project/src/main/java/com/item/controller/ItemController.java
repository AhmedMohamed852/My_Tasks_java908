...
public class ItemController extends HttpServlet {
	...
	//http://localhost:8080/item-service-project/ItemController_java?Action=addItemDetails
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		break;
		case "addItemDetails": addItemDetails(request ,response); 
		
		break;
		case "getItemDetails": getItemDetails(request ,response); 
		
		break;
		case "showItemDetails": showItemDetails(request ,response); 
		
		break;
		case "updateItemDetails": updateItemDetails(request ,response); 
		
		break;
		case "getAndupdateItemDetails": getAndupdateItemDetails(request ,response); 
	...
	private void getAndupdateItemDetails(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			request.getRequestDispatcher("/UbdateItemDetails.jsp").forward(request, response);
	...
	private void updateItemDetails(HttpServletRequest request, HttpServletResponse response) {
		String ManufactureStr = request.getParameter("manufacture_date");
		String expiryStr = request.getParameter("expiry_date");
		
		Date ManuDate =Date.valueOf(ManufactureStr);
		Date expirDate =Date.valueOf(expiryStr);
		
		String description = request.getParameter("description");
		long id = Long.parseLong(request.getParameter("id"));

		Item item = new Item(ManuDate , expirDate ,description ,id);
		
		boolean result =  itemservice.updateItemDetails(item);
		
		
		getItems(request ,response );
		
				
		//item =	itemservice.updateItemDetails(id);
	...
	private void showItemDetails(HttpServletRequest request, HttpServletResponse response) {
		long id = Long.parseLong(request.getParameter("id"));
		Item item = itemservice.showItemDetails(id);
		
		
		request.setAttribute("showItem" ,item);
		
		try {
			request.getRequestDispatcher("/ShowItemDetails.jsp").forward(request, response);
		} catch (ServletException | IOException e) {
			System.out.println("---> " + e.getMessage());
		} 
	}
	...
	private void addItemDetails(HttpServletRequest request, HttpServletResponse response) {
		...
		String expDateStr = request.getParameter("expiry_date");

		Date manuDate = Date.valueOf(manuDateStr);
		...
		Item item = new Item(manuDate ,expDate ,description,id_item );
		
		boolean result = itemservice.addItemDetails(item);
		if(result)
		{getItems(request ,response);}
		else {
...
//_____________________________________________________________________________________
	private void getItemDetails(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			request.getRequestDispatcher("/AddItemDetails.jsp").forward(request, response);
		} catch (ServletException | IOException e) {
			System.out.println("---> " + e.getMessage());
		} 
